...
Улучшения относительно предыдущей реализации:

1. Корректная обработка пассивного залога
   • nsubj:pass → PATIENT (а не AGENT!)
   • obl:agent  → AGENT (истинный агент при пассиве)

2. Анализ одушевлённости через pymorphy2 + морфологические признаки
   • одушевлённый nsubj  → AGENT
   • неодушевлённый nsubj → FORCE (каузатор без воли)

3. Новые семантические роли
   • FORCE       — неодушевлённый причинитель действия
   • INSTRUMENT  — орудие действия (творит. пад., без предлога, неодуш.)
   • COMITATIVE  — совместность (с + творит. пад., одуш.)
   • SOURCE      — источник / исходная точка (из, от, с+Gen)
   • GOAL        — направление / конечная точка (к/ко, в/на+Acc)
   • EXPERIENCER — переживающий при экспериентивных глаголах
   • STIMULUS    — стимул при экспериентивных глаголах
   • BENEFICIARY — бенефициар (косвенный неодуш. объект)

4. Полное извлечение именных групп
   • поле word включает amod/nummod/det-модификаторы
   • поле lemma остаётся только головной леммой (+ flat-части)

5. Разделение obl по падежу
   • Ins без предлога → INSTRUMENT / COMITATIVE / MANNER
   • в/на + Acc      → GOAL (не LOCATION)
   • из/от           → SOURCE
   • с+Gen/Acc       → SOURCE

6. Экспериентивные глаголы
   • nsubj при них   → STIMULUS
   • Dat-аргумент    → EXPERIENCER

7. Разделение аргументов при однородных предикатах
   Если два глагола соединены conj и у второго нет агента — добавляется
   «общее» ребро от агента первого предиката.

8. Pro-drop улучшен
   • пассивные глаголы пропускаются (не добавляется подразумеваемый агент)
"""
